...
const closeButton = addPlayerModal ? addPlayerModal.querySelector('.close-button') : null; // Sjekk om modal finnes
...
function renderOnPitchList() {
    ...
    const sortedPlayers = playerIdsOnPitch
        .map(id => getPlayerById(id)).filter(p => p)
...
function addDragListenersToSquadItems() {
    ...
    items.forEach(item => {
        item.removeEventListener('dragstart', handleDragStart); item.addEventListener('dragstart', handleDragStart);
        item.removeEventListener('dragend', handleDragEnd); item.addEventListener('dragend', handleDragEnd);
    });
}
...
function addDragListenersToBenchItems() {
    ...
    items.forEach(item => {
        item.removeEventListener('dragstart', handleDragStartBench); item.addEventListener('dragstart', handleDragStartBench);
        item.removeEventListener('dragend', handleDragEnd); item.addEventListener('dragend', handleDragEnd);
    });
}
...
function handleDragStart(event) {
    ...
    draggedElement = event.target; dragSource = 'squad';
    try { event.dataTransfer.setData('text/plain', draggedPlayerId); console.log("handleDragStart: dataTransfer.setData satt for", draggedPlayerId); }
...
function handleDragStartBench(event) {
    ...
    draggedElement = event.target; dragSource = 'bench';
    event.dataTransfer.setData('text/plain', draggedPlayerId); event.dataTransfer.effectAllowed = 'move';
    setTimeout(() => { if(draggedElement) draggedElement.classList.add('dragging') }, 0);
}
...
function handleDragStartPiece(event) {
    ...
    draggedElement = pieceElement; dragSource = 'pitch';
    event.dataTransfer.setData('text/plain', draggedPlayerId); event.dataTransfer.effectAllowed = 'move';
    setTimeout(() => { if(draggedElement) draggedElement.classList.add('dragging') }, 0);
...
function handleDragOver(event, targetType) {
    ...
    else if (targetType === 'squad') targetElement = squadListContainer; // Sjekk for tropp-liste
...
function handleDropOnPitch(event) {
    ...
    if (playersOnPitch[playerId]) {
        ...
        if (dragSource === 'bench') {
            const benchIndex = playersOnBench.indexOf(playerId); if (benchIndex > -1) { playersOnBench.splice(benchIndex, 1); }
        }
        stateChanged = true;
    }
...
function handleDropOnSquadList(event) {
    ...
    if (dragSource === 'pitch') {
        if (playersOnPitch[playerId]) {
            ...
            console.log(`Moved player ${playerId} from pitch to squad list`); stateChanged = true;
        }
    } else if (dragSource === 'bench') {
        const benchIndex = playersOnBench.indexOf(playerId);
        if (benchIndex > -1) {
            playersOnBench.splice(benchIndex, 1);
            console.log(`Moved player ${playerId} from bench to squad list`); stateChanged = true;
        }
...
function handleDragEnd(event) {
    setTimeout(() => {
        ...
        if(pitchElement) pitchElement.classList.remove('drag-over');
        if(benchElement) benchElement.classList.remove('drag-over');
        if(squadListContainer) squadListContainer.classList.remove('drag-over'); // Fjern også herfra
        resetDragState();
    }, 0);
}

function resetDragState() {
    draggedPlayerId = null; draggedElement = null; dragSource = null;
}
...
function saveSquad() {
    ...
    try {
        ...
        console.log("saveSquad: Lagring i localStorage ser ut til å ha gått bra.");
...
function loadSquad() {
    ...
    if (savedSquadJson) {
        try {
            ...
            squad = parsedSquad;
            console.log("loadSquad: Parsed squad successfully:", squad);
            const maxId = squad.reduce((max, p) => {
                const idNum = p.id && typeof p.id === 'string' ? parseInt(p.id.split('-')[1]) : 0;
                return Math.max(max, !isNaN(idNum) ? idNum : 0);
            }, 0);
            ...
            return true;
        } catch (e) { console.error("Feil ved parsing av lagret tropp:", e); squad = []; localStorage.removeItem(STORAGE_KEY_SQUAD); return false; }
    }
...
function getCurrentStateData() {
    ...
    for (const playerId in playersOnPitch) {
        const player = getPlayerById(playerId);
        if (player) { playersOnPitchData[playerId] = { x: player.position.x, y: player.position.y, borderColor: player.borderColor }; }
    }
...
function loadLastState() {
    ...
    if (savedState) {
        try { const stateData = JSON.parse(savedState); applyState(stateData); console.log("Siste tilstand lastet."); }
...
function handleSaveSetup() {
    ...
    const currentSetups = getSavedSetups(); const currentState = getCurrentStateData();
...
function handleLoadSetup() {
    ...
    const savedSetups = getSavedSetups(); const setupToLoad = savedSetups[selectedName];
...
function populateSetupDropdown() {
    if (!loadSetupSelect) return;
    const savedSetups = getSavedSetups(); const setupNames = Object.keys(savedSetups);
    loadSetupSelect.innerHTML = '<option value="">Velg oppsett...</option>';
    setupNames.sort();
    setupNames.forEach(name => { const option = document.createElement('option'); option.value = name; option.textContent = name; loadSetupSelect.appendChild(option); });
}
// === 6. Lokal Lagring END ===
