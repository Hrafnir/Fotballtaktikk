...
// NY Referanse for slippmål på banelisten
...
function renderUI() { /* ... (som før V#102) ... */ renderOnPitchList(); renderBench(); renderSquadList(); if(onPitchCountElement) onPitchCountElement.textContent = Object.keys(playersOnPitch).length; if(onBenchCountElement) onBenchCountElement.textContent = playersOnBench.length; }
// === FUNKSJON: renderUI END ===
// === FUNKSJON: renderOnPitchList START ===
function renderOnPitchList() { // Inkluderer endringer fra V#102 (drag+dblclick)
    ...
    sortedPlayers.forEach(player => {
        ...
        listItem.addEventListener('dragstart', handleDragStartOnPitchList);
...
function renderBench() { // Inkluderer endringer fra V#102 (dblclick)
...
function renderSquadList() { // Inkluderer endringer fra V#102 (dblclick)
...
function renderFullSquadList() { // Beholder debug-logging fra V#100
    console.log("renderFullSquadList: Kjører..."); 
    if (!fullSquadListContainer) { console.error("renderFullSquadList: FEIL - Container #full-squad-list-container ikke funnet."); return; }
    console.log("renderFullSquadList: Container funnet:", fullSquadListContainer); 
    fullSquadListContainer.innerHTML = ''; 
    console.log(`renderFullSquadList: Antall spillere i squad: ${squad.length}`); 
    if (squad.length === 0) { console.log("renderFullSquadList: Troppen er tom, viser melding."); fullSquadListContainer.innerHTML = '<p>Ingen spillere i troppen enda. Legg til spillere via sidepanelet.</p>'; return; }
    console.log("renderFullSquadList: Starter tabellbygging..."); 
    const sortedSquad = [...squad].sort((a, b) => a.name.localeCompare(b.name));
    const table = document.createElement('table'); table.style.width = '100%'; table.style.borderCollapse = 'collapse';
    const thead = table.createTHead(); const headerRow = thead.insertRow(); const headers = ['Navn', 'Hovedposisjon', 'Status', 'Handlinger'];
    ...
    sortedSquad.forEach(player => {
        ...
        const roleCell = row.insertCell(); roleCell.textContent = player.mainRole || '-'; roleCell.style.padding = '8px';
        const statusCell = row.insertCell(); statusCell.textContent = PLAYER_STATUSES[player.status] || player.status; statusCell.style.padding = '8px';
        if (player.status === 'INJURED_SHORT' || player.status === 'INJURED_LONG') { statusCell.style.color = 'orange'; } else if (player.status === 'SUSPENDED' || player.status === 'UNAVAILABLE') { statusCell.style.color = 'red'; } else if (player.status === 'AVAILABLE') { statusCell.style.color = 'green'; }
        const actionsCell = row.insertCell(); actionsCell.style.padding = '8px'; const editButton = document.createElement('button'); editButton.textContent = 'Rediger'; editButton.style.padding = '4px 8px'; editButton.addEventListener('click', () => openPlayerDetailModal(player.id)); actionsCell.appendChild(editButton);
    });
    console.log("renderFullSquadList: Tabell bygget, legger til i container..."); 
    fullSquadListContainer.appendChild(table);
    console.log("renderFullSquadList: Ferdig."); 
}
...
function handleDropOnOnPitchList(event) {
    ...
    if (!playerId) { console.warn("Drop on OnPitchList: Mottok tom playerId."); resetDragState(); return; }

    const player = getPlayerById(playerId);
    if (!player) { console.error("Drop on OnPitchList: Fant ikke spiller ID:", playerId); resetDragState(); return; }

    // Sjekk om spilleren allerede er på banen (kan skje ved feil drag/drop)
    if (playersOnPitch[playerId]) {
        console.log(`DropOnOnPitchList: Spiller ${playerId} er allerede på banen.`);
        resetDragState();
        return;
    }

    // Sjekk om banen er full
    ...
    if (dragSource === 'bench' || dragSource === 'squad') {
        console.log(`Flytter spiller ${playerId} fra ${dragSource} til banen (via liste)`);
        
        // Bestem standardposisjon (f.eks. 50%, 50%)
        const defaultX = 50;
        const defaultY = 50;
        player.position = { x: defaultX, y: defaultY };

        // Lag og legg til brikke
        ...
        if (pitchSurface) {
            ...
            // Fjern fra kildeliste hvis det var benken
            if (dragSource === 'bench') {
            ...
            console.error("FEIL: pitchSurface ikke funnet ved plassering fra liste!");
        }
    } else {
        console.log(`DropOnOnPitchList: Ignorerer drag fra kilde: ${dragSource}`);
    }
...
function handleDropOnBench(event) { // Inkluderer endringer fra V#102
    event.preventDefault();
    ...
    if (dragSource === 'pitch' || dragSource === 'onpitch-list') { 
        ...
        if (playersOnPitch[playerId]) {
            console.log(`Flytter spiller ${playerId} fra bane (via ${dragSource}) til benk.`);
            playersOnPitch[playerId].remove(); 
            delete playersOnPitch[playerId];   
            stateChanged = true;
        } else {
             console.warn(`DropOnBench: Spiller ${playerId} dratt fra ${dragSource}, men ikke funnet i playersOnPitch.`);
        }
    }
...
function handleDropOnSquadList(event) { // Inkluderer endringer fra V#102
    event.preventDefault();
    ...
    if (dragSource === 'pitch' || dragSource === 'onpitch-list') { 
        if (playersOnPitch[playerId]) {
            console.log(`Flytter spiller ${playerId} fra bane (via ${dragSource}) til tilgjengelig tropp.`);
            playersOnPitch[playerId].remove(); 
            delete playersOnPitch[playerId];   
            stateChanged = true;
        } else {
             console.warn(`DropOnSquadList: Spiller ${playerId} dratt fra ${dragSource}, men ikke funnet i playersOnPitch.`);
        }
    } else if (dragSource === 'bench') { 
        const benchIndex = playersOnBench.indexOf(playerId);
        if (benchIndex > -1) {
            console.log(`Flytter spiller ${playerId} fra benk til tilgjengelig tropp.`);
            playersOnBench.splice(benchIndex, 1);
            stateChanged = true;
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Hent referanser (inkluderer onPitchSectionElement)
    ...
    benchElement = document.getElementById('bench'); // Dette er div#bench
    ...
    // NYTT: Listener for 'På Banen'-listen/seksjonen
    if (onPitchSectionElement) { 
        ...
        console.log("Listeners: onPitchSectionElement OK"); 
    } else { console.error("onPitchSectionElement ikke funnet!"); }
